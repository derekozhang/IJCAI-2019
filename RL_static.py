...
class ReinforcementLearning(object):
    def __init__(self, number, fractionC, fractionM, gameType, mu):
            self.number = number
    ...
    def create_network(self):
        network = Network(number, 4, 'Random')
    ...
    def initial(self):
        ...
        degree = self.matrix.sum(axis = 1) + 30  # the number of neighbors for each agent
...
for net in nets:
    ...
    for gtype in gtypes:
        for r in range(run):
            ...
            #matrix = network.generateNetworks()
            #matrix = network.dynamic_networks(30)

            RLGame = ReinforcementLearning(number, fractionC, fractionM, gtype, mu)
            #dynamic_network = DGame.create_network()
            dynamic_network = RLGame.create_network()
    ...
    np.savetxt('RL_static_schemes_'+net+'.txt', np.array(EpochResults), fmt='%.2f')
